...
@section('content-body')
    ...
    <section class="content">
        <div class="card">
            ...
            <div class="card-body">
                <form action="/penyewa/{{ $penyewa->id }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="mb-3">
                        <label for="nama" class="form-label">Nama</label>
                        <input type="text" class="form-control" name="name" value="{{ $penyewa->name }}"
                            id="nama">
                    </div>
                    <div class="mb-3">
                        <label for="email" class="form-label">Email</label>
                        <input type="text" class="form-control" name="email" value="{{ $penyewa->email }}"
                            id="email">
                    </div>
                    <div class="mb-3">
                        <label for="PhoneNumber" class="form-label">No Hanphone </label>
                        <input type="number" class="form-control" name="phoneNumber" value="{{ $penyewa->phone_number }}"
                            id="phoneNumber">
                    </div>
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-radio">
                            <input class="custom-control-input" type="radio" id="pria" value="pria"
                                @if ($penyewa->jenis_kelamin == 'pria') checked @endif name="jenis_kelamin">
                            <label for="pria" class="custom-control-label">Pria</label>
                        </div>
                        <div class="custom-control custom-radio">
                            <input class="custom-control-input" type="radio" id="wanita" value="wanita"
                                @if ($penyewa->jenis_kelamin == 'wanita') checked @endif name="jenis_kelamin">
