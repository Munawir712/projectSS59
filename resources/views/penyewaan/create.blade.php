        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="/penyewaan" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 @error('penyewa') is-invalid @enderror" name="penyewa"
                            ...
                            @foreach ($penyewa as $item)
                                <option value="{{ $item->id }}" @selected(old('tipe_kamar'))>{{ $item->name }} -
                                    {{ $item->email }}</option>
                            @endforeach
                        </select>
                        @error('penyewa')
                    ...
                    <div class="form-group">
                        <label>Kost</label>
                        <select id="kost" class="form-control select2 @error('kosan') is-invalid @enderror"
                            name="kosan" style="width: 100%;">
                            <option value="">-- Pilih Kamar Kos --</option>
                            @foreach ($kosan as $item)
                                <option value="{{ $item->id }}" @selected(old('kosan'))>{{ $item->no_kamar }} -
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('kosan')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('jumlah_orang') is-invalid @enderror"
                            value="{{ old('jumlah_orang', 1) }}" max="2" name="jumlah_orang" id="jumlah_orang"
                            aria-describedby="emailHelp">
                        <div id="suggestion" class="form-text"></div>
                        @error('jumlah_orang')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('durasi_sewa') is-invalid @enderror"
                            value="{{ old('durasi_sewa', 1) }}" name="durasi_sewa" id="no_kamar"
                            aria-describedby="emailHelp">
                        @error('durasi_sewa')
                            <div class="invalid-feedback">
                                {{ $message }}
...
@section('js')
    <script>
        $(function() {
            ...
            const jumlahOrangSugges = $('#suggestion')
            $('#kost').on('change', function() {
                const kos_id = $(this).val();
                fetch("{{ url('api/kosan?id=') }}" + kos_id).then(response => response.json()).then(
                    data => {
                        jumlahOrangInput.attr('max', data['data']['max_orang'])
                        jumlahOrangSugges.text("Jumlah max " + data['data']['max_orang'])
                    }
